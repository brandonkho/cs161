...
class Client(BaseClient):
    ...
    def revoke(self, user, name):
        ...
        dictionary[path_join(self.username, name)] = [new_random_id, new_random_key_for_value, new_random_key_for_value_mac]
        ...
        message = [sharename, new_random_id, new_random_key_for_value, new_random_key_for_value_mac]
        message_as_string = util.to_json_string(message)
        ...
        for child in share_file:
            if child != user:
                print(child)
                print(user)
                self.storage_server.put(path_join("[SHARE]", self.username, "sharewith", child, name), "[DATA] " + message_as_string)
